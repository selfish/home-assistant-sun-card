...
describe("Helpers", () => {
  describe("nothing", () => {
    it("returns an empty template result", async () => {
      const element = window.document.createElement("test-element") as TemplateResultTestHelper<typeof Helpers.nothing>;
      element.templateResultFunction = Helpers.nothing;
  ...
  describe("renderFieldElement", () => {
    it("returns an empty template when the provided value is undefined", async () => {
      ...
      const element = window.document.createElement("test-element") as TemplateResultTestHelper<typeof Helpers.renderFieldElement>;
      element.templateResultFunction = Helpers.renderFieldElement;
  ...
  describe("isValidLanguage", () => {
    it("returns true the provided language is supported", () => {
      const result = Helpers.isValidLanguage("es");
      expect(result).toBe(true);
    });

    it("returns false the provided language is not supported", () => {
      const result = Helpers.isValidLanguage("notSupported");
  ...
  describe("todayAtStartOfDay", () => {
    it("returns today at the beginning of the day", () => {
      const result = Helpers.todayAtStartOfDay();
  ...
  describe("todayAtEndOfDay", () => {
    it("returns today at the end of the day", () => {
      const result = Helpers.todayAtEndOfDay();
  ...
  describe("findSectionPosition", () => {
    it("returns the correct section position when it has just started", () => {
      ...
      const result = Helpers.findSectionPosition(timeSinceSectionStarted, timeWhenSectionFinishes, sectionSize);
  ...
  describe("findSunProgress", () => {
    it("returns the correct sun progress when the sun position is lower than the start position", () => {
      ...
      const result = Helpers.findSunProgress(sunPosition, startPosition, endPosition);
      expect(result).toBe(0);
    });
  ...
  describe("clamp", () => {
    it("returns the correct value when min and max have the same value", () => {
      const result = Helpers.clamp(50, 50, 20);
