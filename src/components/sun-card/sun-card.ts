...
export class SunCard extends LitElement {
  ...
  static getConfigElement(): HTMLElement {
    return document.createElement(Editor.cardType);
  }
  ...
  public setConfig(config: ISunCardConfig): void {
    ...
    if (newConfig.language && !Helpers.isValidLanguage(newConfig.language)) {
  ...
  public render(): TemplateResult {
    if (this.data?.error) {
      return new ErrorContent(this.config, this.data.error).render();
    }
    ...
    // render components
    return new CardContent(this.config, this.data).render();
  }
  ...
  private calculateSunInfo(sunrise: Date, sunset: Date): TSunInfo {
    ...
    const dayStart = Helpers.todayAtStartOfDay().valueOf();
    const sunriseMs = sunrise.valueOf();
    const sunsetMs = sunset.valueOf();
    const dayEnd = Helpers.todayAtEndOfDay().valueOf();
    ...
    const dawnSectionPosition = Helpers.findSectionPosition(msSinceStartOfDay, msUntillDawn, Constants.SUN_SECTIONS.dawn);
    const daySectionPosition = Helpers.findSectionPosition(msSinceDawn, msOfDaylight, Constants.SUN_SECTIONS.day);
    const duskSectionPosition = Helpers.findSectionPosition(msSinceDusk, msUntillEnd, Constants.SUN_SECTIONS.dusk);
    ...
    sunPercentOverHorizon = Helpers.clamp(0, 100, sunPercentOverHorizon);
